@echo off
REM Warehouse Management System - Start (SQLite, backend + frontend)
setlocal
...
if "%BSTATUS%"=="0" (
    echo [i] Backend already running on port 3000. Skip backend startup.
    set "BACKEND_SKIP=1"
)
if "%BSTATUS%"=="2" (
...
start "WMS-Backend" cmd /k "cd /d "%PROJECT_DIR%backend" && node "%PROJECT_DIR%backend\server.js""
echo [i] Waiting for backend to be ready...
node "%PROJECT_DIR%backend\wms-control.js" wait-up --port 3000 --timeout 30
if errorlevel 1 (
    echo [X] Backend timed out. Check WMS-Backend window or backend\logs\.
    pause
    exit /b 1
)
:backend_ready
